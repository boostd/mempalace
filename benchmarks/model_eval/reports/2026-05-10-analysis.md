...
3. **Cloud-tier ceiling measurement**: attempted, blocked on `ollama signin` (interactive OAuth). Five cloud candidates (`gpt-oss:20b-cloud`, `gpt-oss:120b-cloud`, `qwen3-coder:480b-cloud`, `deepseek-v3.1:671b-cloud`, `kimi-k2:1t-cloud`) sit in `candidates.yaml` with `tier: cloud`. Once authenticated, run with `--candidates cloud --n 30 --tasks all`.
